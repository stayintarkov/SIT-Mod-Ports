...
using LightStruct = LightsStates;
using PlayerInterface = IFirearms;
using CastingClass = GClass617;
...
namespace CombatStances
{
    ...
    public class SetFireModePatch : ModulePatch
    {
        ...
        private static bool Prefix(FirearmsAnimator __instance, Weapon.EFireMode fireMode, bool skipAnimation = false)
        {
            ...
            GClass632.SetFireMode(__instance.Animator, (float)fireMode);
            if (!skipAnimation)
            {
                GClass632.TriggerFiremodeSwitch(__instance.Animator);
            }
            return false;
        }
    }
        ...
        private static void PatchPrefix(Player.FirearmController __instance, Vector3 origin, float ln, Vector3? weaponUp = null)
        {
            ...
            if (player.IsYourPlayer)
            {
                ...
                if (timer >= 60)
                {
                    ...
                    Func<RaycastHit, bool> func_1 = (Func<RaycastHit, bool>)AccessTools.Field(typeof(EFT.Player.FirearmController), "func_2").GetValue(__instance);
